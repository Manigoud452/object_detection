...
# NOTE: newer torchvision versions removed the `pretrained=True` kwarg in favor of `weights=`.
...
# paths
# Use absolute paths based on this file's location so the app works no matter
# what directory you launch `python app.py` from.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
app.config['UPLOAD_PATH'] = os.path.join(BASE_DIR, 'uploads')
app.config['OBJECTS_PATH'] = os.path.join(BASE_DIR, 'objects')

# Make sure these folders actually exist (they may be missing after copying
# the project between drives, since git/zip only kept a placeholder file).
os.makedirs(app.config['UPLOAD_PATH'], exist_ok=True)
os.makedirs(app.config['OBJECTS_PATH'], exist_ok=True)
...
def find():
    ...
    object_=object
    # default so template rendering never fails with NameError if no object was
    # detected/selected below
    new_file_name = None
    if object_:
        half = 0.5
